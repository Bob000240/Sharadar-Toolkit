...
Ranks are computed over whatever population ``score`` is given, so the caller
decides the comparison set — ranking survivors means "best of those that
qualified", ranking the universe means "best overall".
"""
...
class RankMetric:
    ...
    def __post_init__(self) -> None:
        ...
        :raises ValueError: for a blank field, an unregistered direction, or a
            weight that is not finite and positive. Lower-is-better is
            expressed with ``direction="low"`` and never with a negative
            weight, which would break the coverage renormalisation in
            ``Ranking.score``.
        """
...
class Ranking:
    ...
    ``group_by`` names a column to rank within, such as sector, instead of across
    the whole population. ``positive_only`` names fields where a non-positive value
    is undefined rather than extreme: a loss-maker's P/E is masked out of the
    percentile rather than ranked as cheapest.
    """
    ...
    def __init__(
        ...
        *metrics: RankMetric | tuple,
        ...
        :raises ValueError: when no metric is supplied, when ``top_n`` is
            below 1, when ``min_coverage`` falls outside [0, 1], or when a
            field appears in more than one metric.
        """
        if not metrics:
    ...
    def score(self, frame: pd.DataFrame) -> pd.DataFrame:
        ...
        Adds ``{field}_pct`` per metric, direction-adjusted so 100 is always the good
        end. ``coverage`` is the share of requested weight a security carries data for,
        and the blend is renormalised by it, so a security missing one metric is scored
        on the others rather than handed a zero. Below ``min_coverage`` the blend is
        too thin to mean anything and the row is dropped rather than ranked worst.

        Split from ``apply`` because the population a score is computed over and the
        one it is cut from are not always the same.

        :returns: the scored rows, unordered and uncut.
        """
    ...
    def cut(self, frame: pd.DataFrame) -> pd.DataFrame:
        ...
        Ties share the better rank. Kept separate from ``score`` so the cut can be
        taken over a filtered subset of the population the score was computed on.

        :raises KeyError: when ``frame`` carries no ``score`` column.
        """
